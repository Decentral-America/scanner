...
export default function BlockDetail() {
  ...
  const { data: block, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (height) {
        return blockchainAPI.getBlockByHeight(parseInt(height));
      } else if (id) {
    ...
    return (
      <div className="space-y-6">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>
            {error.message || "Failed to load block"}
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
            ...
            <CardContent className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{block.timestamp ? new Date(block.timestamp).toLocaleString() : "N/A"}</p>
                </div>
                <div className="md:col-span-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {block.signature && <CopyButton text={block.signature} label="Copy signature" />}
                  </div>
                </div>
                <div>
        ...
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            Block not found or invalid block identifier provided.
          </AlertDescription>
        </Alert>
      )}
    </div>
