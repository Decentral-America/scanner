...
import { Coins, AlertCircle, Users, Search, TrendingUp, BarChart3 } from "lucide-react";
        ...
        <>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              {asset && (
                <p className="text-gray-600 mt-1">{asset.name || t("unnamedAsset")}</p>
              )}
            </div>
          </div>
          ...
          {error && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {error.message || t("failedToLoadAsset")}
              </AlertDescription>
            </Alert>
          )}
            ...
            <>
              ...
              <Card className="border-none shadow-lg">
                ...
                <CardContent className="text-center">
                  ...
                  <h3 className="mt-4 text-lg font-medium text-gray-900">{t("exploreHolderDistribution")}</h3>
...
function AssetActivityWidget() {
  const { t } = useLanguage();
  const [assetActivity, setAssetActivity] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchAssetActivity = async () => {
      ...
      try {
        ...
        for (const block of blockHeaders.slice(-8)) {
          try {
            ...
            if (fullBlock?.transactions) {
              ...
              for (const tx of fullBlock.transactions) {
                ...
                if ([4, 11].includes(tx.type) && tx.assetId) {
                  ...
                  if (tx.type === 4) {
                    assetStats[tx.assetId].totalAmount += tx.amount || 0;
        ...
        for (const asset of topAssets) {
          try {
            ...
            const details = await blockchainAPI.getAssetDetails(asset.assetId);
      ...
      <CardContent>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {assetActivity.assets.map((asset, index) => (
              <Link
                key={asset.assetId}
              ...
              >
                ...
                <Badge variant="secondary">{asset.txCount} {t("txAbbreviation")}</Badge>
              </Link>
            ))}
          </div>
        </div>
