...
export default function UnconfirmedTxWidget() {
  ...
  const recentTxs: Transaction[] = transactions?.slice(0, 5) || [];
      ...
      <CardContent>
        ...
        <div className="space-y-3">
          {isLoading ? (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="font-mono text-sm font-medium truncate">{truncate(tx.id, 12)}</p>
                    <p className="text-xs text-gray-500 mt-1">Type: {tx.type}</p>
                  </div>
            ...
            <p className="text-center text-gray-500 py-4 text-sm">No unconfirmed transactions</p>
          )}
        </div>
      </CardContent>
    </Card>
